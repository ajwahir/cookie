...
class ProstheticsEnv(OsimEnv):
    ...
    def get_observation(self):
        ...
        for body_part in ["pelvis", "head","torso","toes_l","pros_foot_r","talus_l","pros_tibia_r","tibia_l","femur_l","femur_r","calcn_l"]:
            ...
            cur += state_desc["body_vel_rot"][body_part][0:3]
            cur += state_desc["body_acc_rot"][body_part][0:3]
            if body_part == "pelvis":
                pelvis = cur
                #intitailly it was res += cur[1:] ---- not sure why x position of the pelvis is never used
                res += cur
            else:
                cur_upd = cur
                # print(len(cur))
                ...
                #the relative values are not used - I will let the neural network approximate that ?
                res += cur
